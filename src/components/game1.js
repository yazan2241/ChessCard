...
import Epiece from '../pieces/Epiece';
...
export default class Game1 extends React.Component {
  ...
  countDown() {
    ...
    if (seconds == 0 && this.countCards !== 4) {
      /*
      const e = this.props.game.aiMove(0);
      const from = Object.keys(e);
      const to = Object.values(e);
      const arr1 = this.getSquareStringR(from[0]);
      const arr = this.getSquareStringR(to[0]);
      console.log(from[0]);
      console.log(to[0]);
      this.state.squares[arr] = this.state.squares[arr1];
      this.state.squares[arr1] = null;

      const e1 = this.props.game.aiMove(4);
      const from1 = Object.keys(e1);
      const to1 = Object.values(e1);
      const arr11 = this.getSquareStringR(from1[0]);
      const arr12 = this.getSquareStringR(to1[0]);
      console.log(from1[0]);
      console.log(to1[0]);
      this.state.squares[arr12] = this.state.squares[arr11];
      this.state.squares[arr11] = null;
*/
  ...
  handleClick(i) {
   ...
   // console.log(this.getKingPosition(this.state.squares , 2))
    if (this.state.sourceSelection === -1) {
      ...
      if (!this.state.squares[i] || this.state.squares[i].player !== this.state.player) {
        ...
        /*
        if(i>63){
          if(this.state.squares[i].constructor.name === "Pawn"){
            for(let j = 48 ; j<=55 ; j++){
              if(this.state.squares[j] === null){
                this.state.squares[j] = new Epiece(1);
                this.state.squares[j].style = { ...this.state.squares[j].style, backgroundColor: "RGB(111,143,114)" };        
              }
            }
            this.state.type = "EPawn";
          }
          else{
            for(let j = 56 ; j<=63 ; j++){
              if(this.state.squares[j] === null){
          ...
          this.state.type = this.props.game.moves(this.getSquareString(i));
          for( let ii = 0;ii<this.state.type.length ; ii++){
            if(this.state.squares[this.getSquareStringR(this.state.type[ii])] === null){
              this.state.squares[this.getSquareStringR(this.state.type[ii])] = new Epiece(1);
              this.state.squares[this.getSquareStringR(this.state.type[ii])].style = { ...this.state.squares[this.getSquareStringR(this.state.type[ii])].style, backgroundColor: "RGB(111,143,114)" };
            }
          }
        }
        */
    ...
    /*
    if(this.state.type === "EPawn"){
      for(let j = 48 ; j<=55 ; j++){
        if(this.state.squares[j].constructor.name === "Epiece"){
          this.state.squares[j].style = { ...this.state.squares[j].style, backgroundColor: "" };        
          this.state.squares[j] = null;
        }
      }
      this.state.type = "";
    }
      ...
      for( let ii = 0;ii<this.state.type.length ; ii++){
        if(this.state.squares[this.getSquareStringR(this.state.type[ii])].constructor.name === "Epiece"){
          this.state.squares[this.getSquareStringR(this.state.type[ii])].style = { ...this.state.squares[this.getSquareStringR(this.state.type[ii])].style, backgroundColor: "" };        
          this.state.squares[this.getSquareStringR(this.state.type[ii])] = null;
        }
      }
      this.state.type = ""
    }
    */
    ...
    if (this.state.sourceSelection > 63) {
          if (i > 48 && i < 65) {
            ...
            if (this.state.squares[this.state.sourceSelection].constructor.name === 'Pawn') {
                //console.log("Pawn added "+ this.getSquareString(i));
              ...
              if (this.state.enemyPlayerCard !== 0 && this.state.aiTurn1) {
               ...
               this.props.game.removePiece(this.getSquareString(fii));
                const e = this.props.game.aiMove(4);
              ...
              let r1=1;
              if(this.state.enemyPlayerCard === 1)r1 = 2;
              if(this.state.enemyPlayerCard === 2)r1 = 4;
              if(this.state.enemyPlayerCard === 3)r1 = 3;
              if(this.state.enemyPlayerCard === 4)r1 = 1;
              
              let tt = r1+64;
              let tt1 = 64 + r1 + 4;
             // console.log("tt "+tt);
             // console.log("sourc "+ this.state.sourceSelection);
              let ele =  this.state.squares[tt].constructor.name;
             // let ele1 =  this.state.squares[this.state.sourceSelection].constructor.name;
            ...
            //  console.log("ai setting piece")
              
                if (this.state.enemyCrad[r1] === 1) {
                  const one = 1;
                  const two = 8;
                  let c=0;
                  let xx;
                  do {
                    xx = one + Math.random() * (two - one);
                    xx = Math.round(xx);
                    c++;
                  } while (this.state.squares[7 + xx] && c!==7 );
                  let ch = this.getChar(xx);
                  ch += '7';
                  this.props.game.setPiece(ch, "p");
                 // console.log("hel " + xx)
                 // console.log("yy " + ch);
                  this.state.squares[7 + xx] = this.state.squares[r1 + 64]
                }
                if (this.state.enemyCrad[r1] === 4) {
                  if (this.state.squares[2] === null) {
                    this.props.game.setPiece("C8", "b");    
                    this.state.squares[2] = this.state.squares[r1 + 64];
                  }
                  else if(this.state.squares[5] === null) {
                    this.props.game.setPiece("F8", "b");
                    
                    this.state.squares[5] = this.state.squares[r1 + 64]
                  }
                  else{
                    for(let x = 0 ;x <8; x++){
                        if(this.state.squares[x] === null){
                          this.props.game.setPiece(this.getSquareString(x), "b");
                          
                          this.state.squares[x] = this.state.squares[r1 + 64]
                          break;
                        }
                    }
                  }
                }
                if (this.state.enemyCrad[r1] === 2) {
                  if (this.state.squares[0] === null) {
                    this.props.game.setPiece("A8", "r");
                    
                    this.state.squares[0] = this.state.squares[r1 + 64]
                  }
                  else if(this.state.squares[7] === null){
                    this.props.game.setPiece("H8", "r");
                   
                    this.state.squares[7] = this.state.squares[r1 + 64]
                  }
                  else{
                      for(let x = 0 ;x <8; x++){
                          if(this.state.squares[x] === null){
                            this.props.game.setPiece(this.getSquareString(x), "r");
                ...
                if (this.state.enemyCrad[r1] === 5) {
                    if (this.state.squares[3] === null) {
                    this.props.game.setPiece("D8", "q");
                 
                  this.state.squares[3] = this.state.squares[r1 + 64]
                    }
                    else{
                        for(let x = 0 ;x <8; x++){
                            if(this.state.squares[x] === null){
                              this.props.game.setPiece(this.getSquareString(x), "q");
                ...
                if (this.state.enemyCrad[r1] === 3) {
                  if (this.state.squares[1] === null) {
                    this.props.game.setPiece("B8", "n");
                    
                    this.state.squares[1] = this.state.squares[r1 + 64]
                  }
                  else if(this.state.squares[6] === null){
                    this.props.game.setPiece("G8", "n");
                    
                    this.state.squares[6] = this.state.squares[r1 + 64]
                  }
                  else{
                    for(let x = 0 ;x <8; x++){
                        if(this.state.squares[x] === null){
                          this.props.game.setPiece(this.getSquareString(x), "n");
               ...
               // console.log("ai played "+this.state.squares[r1 + 64].constructor.name)
                this.state.squares[r1 + 64] = null;
                this.state.enemyPlayerCard--;

                this.setState(oldState => ({
                  sourceSelection: -1,
                  squares: this.state.squares,
                  status: '',
                  enemyPlayerCard: this.state.enemyPlayerCard,
                  aiTurn1: !this.state.aiTurn1,
                  ...
                  /*
                    ...
                    */
                    ...
                    //this.props.game.move(this.getSquareString(fii), this.getSquareString(see));
                    //this.props.game.removePiece(this.getSquareString(see));
                    //console.log("fii is " + fii + " see is " + see)
                    const e = this.props.game.aiMove(4);
                    //console.log(e);
                    const from = Object.keys(e);
                    const to = Object.values(e);
        ...
        try {
            ...
            if(this.props.game.moves(this.getSquareStringR(this.getKingPosition(this.state.squares,this.state.player))).length === 0 ){
              this.state.Winner = "Computer"
              this.endGame = true;
            }
           ...
           this.state.squares[i] = this.state.squares[this.state.sourceSelection];
              ...
              this.props.game.removePiece(this.getSquareString(see));  
                ...
                let tt1 = 64 + r1 + 4;
               // console.log(tt);
                const ele =  this.state.squares[tt].constructor.name;
                //const ele1 =  this.state.squares[this.state.sourceSelection].constructor.name;
               ...
               // console.log("ele "+ele);
                ...
                if (this.state.enemyCrad[r1] === 1) {
                  ...
                  this.state.squares[7 + xx] = this.state.squares[tt]
                }
                ...
                if (this.state.enemyCrad[r1] === 4) {
                  if (this.state.squares[2] === null) {
                    ...
                    this.state.squares[2] = this.state.squares[tt]
                  }
                  else if(this.state.squares[5] === null) {
                    this.props.game.setPiece("F8", "b");
                  
                    this.state.squares[5] = this.state.squares[tt]
                  }
                  else{
                    for(let x = 0 ;x <8; x++){
                        if(this.state.squares[x] === null){
                          ...
                          this.state.squares[x] = this.state.squares[tt]
                          break;
                        }
                    }
                  }
                ...
                if (this.state.enemyCrad[r1] === 2) {
                  if (this.state.squares[0] === null) {
                    ...
                    this.state.squares[0] = this.state.squares[tt]
                  }
                  else if(this.state.squares[7] === null){
                    this.props.game.setPiece("H8", "r");
                    
                    this.state.squares[7] = this.state.squares[tt]
                  }
                  else{
                      for(let x = 0 ;x <8; x++){
                          if(this.state.squares[x] === null){
                ...
                if (this.state.enemyCrad[r1] === 5) {
                  ...
                  this.state.squares[3] = this.state.squares[tt]
                    }
                    else{
                        for(let x = 0 ;x <8; x++){
                            if(this.state.squares[x] === null){
                ...
                if (this.state.enemyCrad[r1] === 3) {
                  if (this.state.squares[1] === null) {
                    ...
                    this.state.squares[1] = this.state.squares[tt]
                  }
                  else if(this.state.squares[6] === null){
                    this.props.game.setPiece("G8", "n");
                  
                    this.state.squares[6] = this.state.squares[tt]
                  }
                  else{
                    for(let x = 0 ;x <8; x++){
                        if(this.state.squares[x] === null){
                ...
                //console.log("ai piece name "+ ele)
                
                //console.log("ai s name " + (this.state.squares[tt]))
                this.state.squares[tt] = null;
                this.state.enemyPlayerCard--;
    ...
    getKingPosition(squares, player) {
      ...
      <div className="New-game">
        <div className="box">
          <p>Winner</p><br/><br/>
         <p style={{color:"#00f"}}> {this.state.Winner}</p>
        <br /><br/>
        <button className="buexit" onClick={()=>{window.FBInstant.quit()}}>Exit</button>
        </div>
        </div>
      }
          </div>
